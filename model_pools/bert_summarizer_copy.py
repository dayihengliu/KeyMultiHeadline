...
class BertSummarizerCopy(BaseModel):
    ...
    def decode_infer(self, inputs, state):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('decode', reuse=True):
                ...
                ret = transformer_concated_decoder(infer_decoder_input,
                                                   self.enc_output,
                                                   infer_dec_attn_bias,
                                                   self.enc_attn_bias,
                                                   self.hps,
                                                   state=state['decoder'],
                                                   src_true_mask=self.input_mask)
    ...
    def _make_input_key(self):
        ...
        self.input_keys_infer = ['source_ids', 'source_len', 'source_seg_ids', 'max_oov_num', 'encoder_output']
