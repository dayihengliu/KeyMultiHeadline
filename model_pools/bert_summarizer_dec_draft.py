...
class BertSummarizerDecDraft(BaseModel):
    """
    Based on BertSummarizerDecV3.
    Using draft summary generated from decoder each n batches.
    """

    def __init__(self, bert_config, batcher, hps):
        super(BertSummarizerDecDraft, self).__init__(hps, bert_config, batcher)
    ...
    def _build_summarization_model(self):
        ...
        mask_percent = self.hps.mask_percentage
        total_percent = self.hps.total_percentage

        def replace_with_mask(seq_tensor):
            batch_size = tf.shape(seq_tensor)[0]
            tgt_len = tf.shape(seq_tensor)[1]
            rnd_value = tf.random_uniform([batch_size, tgt_len], minval=0, maxval=1)
            replace_to_mask = rnd_value < mask_percent
            keep_the_word = rnd_value < total_percent
            keep_the_word = tf.cast(keep_the_word, tf.float32)

            all_mask = tf.ones_like(seq_tensor, dtype=tf.int32)
            mask_id = self.hps.maskId
            all_mask = all_mask * mask_id

            changed_ids = tf.where(replace_to_mask, all_mask, seq_tensor)
            return changed_ids, keep_the_word

        changed_ids, keep_the_word_ground_truth = replace_with_mask(self.output_ids)
        ...
        trun_pred_ids = self.trunct(self.pred_ids)
        changed_pred_ids, keep_the_word_pred = replace_with_mask(trun_pred_ids)
        draft_dec_model = modeling.BertModel(
            config=self.bert_config,
            is_training=is_training,
            input_ids=changed_pred_ids,
            input_mask=self.output_mask,
            token_type_ids=self.out_segment_ids,
            scope='bert',
            reuse=tf.AUTO_REUSE,
            use_one_hot_embeddings=self.hps.use_tpu)  # use_one_hot_embeddings=Flags.tpu ?
        draft_dec_output = draft_dec_model.get_sequence_output()  # [b, l_t, h]
        keep_the_word = tf.cond(self.feed_draft, lambda: keep_the_word_pred, lambda: keep_the_word_ground_truth)

        with tf.variable_scope('decoder'):
            self.out_embed = tf.cond(self.feed_draft, lambda: draft_dec_output, lambda: dec_output)
            self.masked_out_embed = self.out_embed * tf.expand_dims(self.output_mask, -1)
            self.decoder_input = tf.pad(self.masked_out_embed, [[0, 0], [1, 0], [0, 0]])[:, :-1, :]  # Shift left
            # ################################################### decoding train - 2
            self.second_dec_attn_bias = attention_bias(tf.shape(self.masked_out_embed)[1], 'no_bias')
