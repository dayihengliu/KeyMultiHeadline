...
class BertSummarizerDecV4V3(BaseModel):
    """
    ...
    def __init__(self, bert_config, batcher, hps):
        super(BertSummarizerDecV4V3, self).__init__(hps, bert_config, batcher)
    ...
    def _build_summarization_model(self):
        ...
        self.pred_ids = tf.ones_like(self.output_ids)  # [b, l_t]
        ...
        with tf.variable_scope('decoder_2'):
            dec_model = modeling.BertModel(
                ...
                reuse=False,
                use_one_hot_embeddings=self.hps.use_tpu)  # use_one_hot_embeddings=Flags.tpu ?
            self.decoder_weights = dec_model.get_embedding_table()
    ...
    def decode_infer_2_bs(self):
        ...
        with self.graph.as_default():
            ...
            draft = self.trunct(target_sequence)  # as the draft may have copy words, we transform them to UNK first
            target_sequence = self.trunct(target_sequence)
            ...
            tgt_mask = tf.sequence_mask(target_length,
                                        ...
                                        dtype=tf.float32)  # [b, l_t]
            draft = tf.cast(draft, tf.int32)
            changed_ids = tf.concat([target_sequence, draft], axis=-1)  # [b, 2 * l_t]
            change_segment_ids = tf.zeros_like(changed_ids, dtype=tf.int32, name='change_segment_ids')
            ...
            change_ids_mask = tf.concat([tgt_mask, pred_ids_mask], axis=-1)  # [b, 2 * l_t]

            transferred_mask = create_attention_mask_from_input_mask(changed_ids,
                                                                     change_ids_mask)  # [b, 2 * l_t, 2 * l_t]

            second_dec_attn_bias_w_draft = attention_bias(tf.shape(changed_ids)[1], 'mask_draft')
            second_dec_attn_bias = second_dec_attn_bias_w_draft  # [1, 1, 2 * l_t, 2 *l_t]
            second_dec_attn_bias = tf.tile(second_dec_attn_bias,
                                           [tf.shape(target_sequence)[0], 1, 1, 1])  # [b, 1, 2 * l_t, 2 * l_t]

            second_dec_attn_bias = second_dec_attn_bias * tf.expand_dims(transferred_mask,
                                                                         1)  # [b, 1, 2 * l_t, 2 * l_t]
            is_training = self.is_training
            dec_model = modeling.BertModel(
                config=self.bert_config,
                is_training=is_training,
                input_ids=changed_ids,
                input_mask=tf.squeeze(second_dec_attn_bias, 1),  # [b, 2 * l_t, 2 * l_t]
                token_type_ids=change_segment_ids,
                scope='bert',
                reuse=tf.AUTO_REUSE,
            ...
            with tf.variable_scope('decoder_2', reuse=True):
                # [b, l_t, e]
                masked_tgt_embed = tgt_embed * tf.expand_dims(change_ids_mask, -1)
                ...
                second_logits = tf.matmul(decoder_output, self.decoder_weights, False, True)  # (b*beam, v)
    ...
    def decode_infer_2(self):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('decoder_2', reuse=True):
                ...
                # keep only ground-truth part for attention weight & decoder output
                all_att_weights[-1] = all_att_weights[-1][:, :target_len, :]  # [b, l_t, l_s]
                decoder_output = decoder_output[:, :target_len, :]  # [b, l_t, v]
