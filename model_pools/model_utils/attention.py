...
def compute_copy_weights(query, memory, mem_bias, hidden_size, dropout_rate, mem_weight_mask):
    """
    weight_mask is An optional tensor with shape [batch, l_s],
    if is not None, use it calculate individual weight for copy probabilities.
    """
    # query: [b, l_t, e], memory: [b, l_s, e]
    memory = linear(memory, hidden_size, scope="bilinear_m")  # [b, l_s, e]
    memory = memory * tf.expand_dims(mem_weight_mask, -1)  # mask memory  # [b, l_s, e]
    logits = tf.matmul(query, memory, transpose_b=True, name='att_logits')  # [b, l_t, l_s]
    # mem_bias shape is [b, 1, 1, l_s], we need to reshape to [b, 1, l_s]
    mem_bias = tf.squeeze(mem_bias, axis=1)  # [b, 1, l_s]
    logits += mem_bias  # ignore useless part of decode output (tail in l_t)
    weights = tf.nn.softmax(logits, name="attention_weights")  # [b, l_t, l_s]
    if dropout_rate is not None and dropout_rate > 0.0:
        weights = tf.nn.dropout(weights, 1 - dropout_rate)
    return weights
