...
class BertSummarizerPreTrain(BaseModel):
    """
    Based on BertSummarizerDecV4V2.
    Remove Pre-trained BERT from decoder side, and pretrain in Wikipedia.
    """

    def __init__(self, bert_config, batcher, hps):
        super(BertSummarizerPreTrain, self).__init__(hps, bert_config, batcher)
    ...
    def _build_summarization_model(self):
        ...
        self.decode_2()
        self.def_loss()
        self.define_assign_and_fake()

    def define_assign_and_fake(self):
        with tf.variable_scope('bert', reuse=tf.AUTO_REUSE):
            with tf.variable_scope('embeddings'), tf.device('/cpu:0'):
                config = self.bert_config
                pos_emb_var = tf.get_variable(name='position_embeddings',
                                              shape=[config.max_position_embeddings, config.hidden_size])
                self.fake_position_emb = tf.get_variable(name='fake_position_emb',
                                                         shape=[config.max_position_embeddings // 2, config.hidden_size],
                                                         initializer=create_initializer(config.initializer_range))
                self.assign_op = pos_emb_var.assign(tf.concat([self.fake_position_emb, self.fake_position_emb], 0))
    ...
    def def_loss(self):
        with tf.variable_scope('loss'):
            ...
            # self.def_loss = self.hps.rl_lambda * self.rl_loss + (1 - self.hps.rl_lambda) * self.ml_loss
