...
from utils.copy_utils import calculate_two_copy_logits, tf_trunct
...
class TopicEncoderFilterDecoderMemCopy(BaseModel):
    """
    ...
    def __init__(self, bert_config, batcher, hps):
        super(TopicEncoderFilterDecoderMemCopy, self).__init__(hps, bert_config, batcher)
    ...
    def _build_summarization_model(self):
        ...
        for i in range(self.hps.n_gpu):
            ...
            with tf.device(assign_to_gpu(i, "/gpu:0")), tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
                ...
                topic_id_sequence = tf.tile(expand_topic_id, [1, encoder_out_length]) 
                with tf.variable_scope('topic'):
                    with tf.variable_scope('embeddings'), tf.device('/cpu:0'):
                        ...
                        (self.topic_embed, self.topic_embeddings) = embedding_lookup(
                            input_ids=topic_id_sequence,  # here the embedding input of decoder have to be output_ids
                ...
                encoder_output_topic_emb = tf.concat([encoder_output, self.topic_embed], -1)
                ...
                with tf.variable_scope("encoder_topic_attention"):
                    ...
                    y = multihead_attention(
                        layer_process(encoder_output_topic_emb, params.layer_preprocess),
                ...
                with tf.variable_scope('decode'):
                    ...
                    self.all_att_weights1, self.all_att_weights2, self.decoder_output = transformer_decoder_three(self.decoder_input,
                                                                                    self.encoder_output,
                                                                                    self.topic_word_memory,
                                                                                    self.dec_attn_bias,
                                                                                    self.enc_attn_bias,
                    ...
                    with tf.variable_scope('copy'):
                        self.single_logits = calculate_two_copy_logits(self.decoder_output, self.all_att_weights1, self.vocab_probs,
    ...
    def decode_infer(self, inputs, state):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('decode', reuse=True):
                ...
                with tf.variable_scope('copy'):
                    logits = calculate_two_copy_logits(decoder_output, all_att_weights1,
                                                    vocab_probs,
                                                    self.input_ids_oo, self.max_out_oovs, self.input_mask, vocab_size,
                                                    1, all_att_weights2, self.topic_words_ids, self.topic_words_mask)
    ...
    def _make_input_key(self):
        ...
        self.tensor_list = {'source_ids': self.input_ids,
                            ...
                            'all_att_weights': self.all_att_weights1
                            }
        if self.is_training:
        ...
        self.input_keys_infer = ['source_ids', 'topic_words_ids', 'topic_words_len', 'topic_ids', 'source_ids_oo', 'source_len', 'source_seg_ids', 'max_oov_num', 'encoder_output', 'topic_word_memory', 'mem_segment_ids']
        self.input_keys = ['source_ids', 'topic_words_ids', 'topic_words_len', 'source_ids_oo', 'topic_ids', 'source_len', 'source_seg_ids', 'max_oov_num', 'target_ids', 'target_ids_oo', 'target_len', 'mem_segment_ids']
