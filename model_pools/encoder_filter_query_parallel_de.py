...
from model_pools.model_utils.module import smooth_cross_entropy, transformer_decoder, transformer_decoder_four
...
class QueryEncoderFilterParallelDecoderMem(BaseModel):
    """
    ...
    def __init__(self, bert_config, batcher, hps):
        super(QueryEncoderFilterParallelDecoderMem, self).__init__(hps, bert_config, batcher)
    ...
    def _add_placeholders(self):
        ...
        self.enc_output_origin = tf.placeholder(tf.float32, [None, None, self.hps.hidden_size], name='enc_output_origin')
    ...
    def _build_summarization_model(self):
        ...
        for i in range(self.hps.n_gpu):
            ...
            with tf.device(assign_to_gpu(i, "/gpu:0")), tf.variable_scope(tf.get_variable_scope(), reuse=do_reuse):
                ...
                self.encoder_output_origin = encoder_output
                ...
                with tf.variable_scope('decode'):
                    ...
                    self.all_att_weights, _, self.decoder_output = transformer_decoder_four(self.decoder_input,
                                                                                    self.encoder_output_origin,
                                                                                    self.topic_word_memory,
                                                                                    self.encoder_output,      
                                                                                    self.dec_attn_bias,
                                                                                    self.enc_attn_bias,
    ...
    def decode_infer(self, inputs, state):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('decode', reuse=True):
                ...
                ret = transformer_decoder_four(infer_decoder_input,
                                          self.enc_output_origin,
                                          self.topic_memory,
                                          self.enc_output,
                                          infer_dec_attn_bias,
                                          self.enc_attn_bias,
    ...
    def _make_input_key(self):
        ...
        self.tensor_list = {'source_ids': self.input_ids,
                            ...
                            'encoder_output_origin': self.enc_output_origin,
        ...
        self.input_keys_infer = ['source_ids', 'topic_words_ids', 'topic_words_len', 'source_ids_oo', 'source_len', 'source_seg_ids', 'max_oov_num', 'encoder_output', 'encoder_output_origin', 'topic_word_memory', 'mem_segment_ids']
